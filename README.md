...
**主線是填表許願，其餘意見一律不問身分。** 收集意見本身才是目的，知道是誰不是。
少一個 email 欄就少一道摩擦，而摩擦正是現階段最貴的東西。

| 管道 | 身分 | 何時 | 用途 |
|---|---|---|---|
| **許願表單** | **email 必填** | 隨時 | 主線。需求來源，也是閉環的對象 |
| 💬 給建議 | 無 | 隨時，對任何一筆 | 開發**前**修正方向 |
| 🧪 願意幫測 | 無 | 隨時 | 純表態，計入排序 |
| 意見回饋 | 無 | 完成後 | 開發**後**看做得對不對 |

**閉環不受影響**：確認信承諾的對象是**許願者**，而許願者的 email 本來就必填。
其他人匿名不影響那個承諾。

**代價**（刻意接受）：做完之後**約不到特定的人來測**，只能被動等意見，也無法追問。
現階段的瓶頸是「有沒有人願意開口」，不是「意見夠不夠深」。

意見表單只問三題，不分角色：

1. **試用了嗎**（有／沒空試／打不開）← 過濾題，沒試用就不問後面
2. 哪裡卡住、哪裡跟你想的不一樣
3. 最想補什麼

`suggestions` 分頁只存 `wishId` / `text` / `createdAt`，**無任何身分欄**。

#### 開發中的通知流程

```
已評估(可行) → 排程中 → 開發中 ──> 已完成 ──> 寄信給許願者（demo + 意見表單）
                                                    ↓
                                                 反饋中 ──> 意見寫回 Notion
                                                    ↓
                                                 迭代中 ──> 下一版完成 ──> 再通知一次
...
**「開發中」刻意不寄通知。** 開發可能拖很久，中途報平安等於製造期待然後失約——
寧可完成時一次給足。想看進度的人去看公開列表，那是拉不是推。

#### 曝光與冷啟動

**不在目前階段。** 沒有第一筆許願整個作品的前提就不成立，但那是上線之後的事。
路徑與判斷記在根目錄 `ideas.md` 的 **I-006**（含「為什麼廣播幾乎一定是 0」、
1 對 1 邀請的順序、以及為什麼要先有已完成案例再發文）。
