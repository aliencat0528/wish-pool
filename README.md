...
匿名點 🧪 只會得到一個數字，**沒有任何方式聯絡到那個人**——訊號拿不到手。
所以點 🧪 時就地展開一個 email 欄：

- **選填**。不填 → 仍計入匿名票數，只是完成時通知不到他
- 填了 → 存進 Sheet 的 `testers` 分頁（`wishId` / `email` / `joinedAt` / `unsubscribed`）
- **email 一律不進 Notion**（同 WP-002）。Notion 只放 `願測人數` 這個數字

#### 反饋的形式：依角色分兩套

許願者與願測者能回答的問題**不一樣**，用同一張表會兩邊都問不到重點。
表單帶參數 `?w=WISH-0001&r=wisher|tester`：

| | 共同題 | 角色專屬題 |
|---|---|---|
| **許願者** | ① 試用了嗎（有／沒空／打不開）<br>② 哪裡卡住<br>③ 最想補什麼 | ④ **有沒有解決你原本的痛點**（完全／部分／沒有）<br>—— 表單上直接顯示他當初寫的痛點原文供對照<br>⑤ 跟你想像的呈現效果差多少 |
| **願測者** | 同上 | ④ **你會自己用嗎**（會／不會／看情況）<br>—— 對非需求方最誠實的問題<br>⑤ 第一次打開有哪裡看不懂 |

第 ① 題是過濾題：**沒試用就別問後面**，否則會收到一堆憑印象的空話。

<!-- 為什麼分兩套：許願者知道痛點有沒有被解，但他對介面已經有預期、看不出新手障礙；
     願測者剛好相反。混在一起問，兩種洞察都會被稀釋成「還不錯」。 -->

#### 開發中的通知流程

```
已評估(可行) → 排程中 → 開發中 ──> 已完成 ──> 寄測試邀請（demo + 反饋表單）
                                                    ↓
                                                 反饋中 ──> 反饋寫回 Notion
                                                    ↓
                                                 迭代中 ──> 下一版完成 ──> 再通知一次
...
**「開發中」刻意不寄通知。** 開發可能拖很久，先寄等於製造期待然後失約——
寧可完成時一次給足，也不要中途報平安。進度想看的人去看公開列表。

收件對象：該筆的**許願者**（Sheet 主表）＋ 所有 **願測者**（`testers` 分頁，
排除 `unsubscribed`）。每封信都要有退訂連結。
